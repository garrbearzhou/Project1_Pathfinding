...
a timer to allow the user to track how long each program takes for certain setups, a heading above the actual grid showing the current heuristic and time with a line separating the grid and the header, and the ability for the user to draw both walls (the algorithm must go around) and mud (cost is 3x more than a normal node)
