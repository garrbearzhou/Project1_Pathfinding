...
to create walls that the program must navigate around. While running, the program will use A* to find the fastest/most efficient
...
Pathfinding is a simple and applicable example of Machine Learning. The program learns the best path towards the goal by itself.
...
a timer to allow the user to track how long each program takes for certain setups, 
and a heading above the actual grid showing the current heuristic and time with a line separating the grid and the header.
...
Building on that, a slightly more advanced pathfinding algorithm is called Dijkstra. Dijkstra builds on BFS by incorporating edge weights into it's cost-benefit analysis
...
There are 3 main types of heuristics: Diagonal (Chebyshev, Manhattan, Euclidean). The diagonal heuristic can go in 8 directions like a king on a chessboard and the formula to calculate 
...
heuristic is the most complex one. It can go anywhere in a sphere of radius one, but on a 2D plane it has the same functionality as the diagonal heuristic. It is calculated by:
...
a higher estimated cost to get to the goal. Euclidean is the most complex to implement but the most efficient in a 3D environment
